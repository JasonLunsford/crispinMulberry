General (On-going) Notes
	...
	11) Had trouble formatting the Recent Articles text, couldn't get the right balance between width and font-size. Noticed the <p> elements were occupying nearly 100px by default anyway, so I formalized that height and set a line height that helped get the font-size effect without violating the left margin on the image or unnaturally squeezing the <p> element.
		A) I decided to use pixels for the fonts in this project since that's what I'm comfortable with. However, I did notice em's (while not perfect either), gave me a different measuring scale entirely. Very useful to remember in the future.
